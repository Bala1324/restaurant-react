...
router.post('/table-book', async (req, res) => {
    try {
        ...
        if (location == 'Patio') {
            ...
            for (let i = 0; i <= 10; i++) {
                ...
                if (findData.length != 0) {
                    if (i == 10) {
                        console.log(i)
                        return res.status(200).json({ status: true, message: "block", i })
                    }
                    continue;
                } else {
                    table = insideTable;
            ...
            for (let i = 0; i <= 10; i++) {
                ...
                if (findData.length != 0) {
                    if (i == 10) {
                        console.log(i)
                        return res.status(200).json({ status: false, message: "block", i })
                    }
                    continue;
                } else {
                    table = BarTable;
        ...
        const data = new tableBookingSchema({ date, time, location, person, table });
